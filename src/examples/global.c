...
double * hmin = (double [NL]){ 0, 40, 40, 40, 40, 50 };
...
int main (int argc, char * argv[])
{
  ...
  /**
  The default resolution is 768 (longitude) x 256 (latitude)
  i.e. 120/256 $\approx$ 1/2 of a degree. */
...
void load_wind (vector wind, int index)
{
  ...
  input_grd (wind.x, file = name, linear = true);
  sprintf (name, "coads-%d_6.asc", index + 1);
  input_grd (wind.y, file = name, linear = true);
#else // HR
  sprintf (name, "wind/hr-%d-x.asc", index + 1);
  input_grd (wind.x, file = name, linear = true, smooth = 1);
  sprintf (name, "wind/hr-%d-y.asc", index + 1);
  input_grd (wind.y, file = name, linear = true, smooth = 1);
#endif // HR
}
